...
from fishproviz.config import STIME
...
def cluster_motion_video(wshedfile, parameters, cluster_id, rows=2, cols=2):
    ...
    for fk in get_camera_pos_keys():
        for day in get_all_days_of_context():
    for s,e,score in results[cluster_id]:
        try: 
            fk, day, start, end = get_fish_info_from_wshed_idx(wshedfile,s,e)
            seqences_of_cid.append((fk, day, start, end, score))
        except ValueError as e:
            print(e)
            
    fig, axes = plt.subplots(2, 2, figsize=(10,10))
    update_functions = list()
    for (fk, day, start, end, score),ax in zip(seqences_of_cid, axes.flatten()):
